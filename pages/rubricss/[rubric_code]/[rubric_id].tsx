...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  try {
    const response = await axios.get(
      `${serverUrl}/rubric/${rubric_id}?limit=10&page=1`
    );
    //@ts-ignore
...
const Rubrics: NextPage<{ rubrics: IValue[]; info: IInfo }> = ({
  rubrics,
  info,
}) => {
  ...
  const [newRubrics, setNewRubrics] = useState<IValue[]>([]);
    ...
    try {
      ...
      if (currentPage <= maxPage) {
        const response = await axios
          .get(`${serverUrl}/rubric/${rubric_id}?limit=10&page=${currentPage}`)
          .finally(() => setFetching(false));

        const responseRubrics: IValue[] = Object.values(
          response.data.message[rubric_code as string]
        );
            ...
            <div className="layout__main">
              <div className="layout__main-wrapper">
                <div className="layout__center">
                  ...
                  {/*    </div>*/}
                  {/*</div>*/}
                  {(newRubrics.length === 0 ? rubrics : newRubrics).map(
                    (rubric) => {
                      const detailPicture = rubric.PICTURE;
                      const originalImage =
                        detailPicture && detailPicture.ORIGINAL;
                      return (
                        <div
                          key={rubric.ID}
                          onClick={() =>
                            redirectToOtherPage(rubric.TYPE, rubric.ID)
                          }
                        ...
                        >
                          <div className="big-news-card__body">
                            ...
                            <a className="big-news-card__title">
                              {rubric.NAME}
                            </a>
                          </div>
                          {originalImage && (
                            <a href="#" className="big-news-card__img">
                              <img
                                src={`${server}${rubric.PICTURE.ORIGINAL}`}
                                alt="Image"
                              />
                            </a>
                          )}
                    ...
                    <a href="#" className="video-widget layout__sticky">
                      <div className="video-widget__body">
                        <div className="video-widget__inner">
                          <span className="video-widget__title">
                            Какие-то видео
                          </span>
                        ...
                        <span className="video-widget__help">
                          35 новых видео
                        </span>
                      </div>
