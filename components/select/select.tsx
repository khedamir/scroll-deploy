  ...
  return (
    <div className="select-box">
      <div className="select-result">
        ...
        <div className="select-result__value">{selectRubric && selectRubric.NAME}</div>
      </div>
      <button onClick={handleClickSelect} className={`select-btn ${isOpen && 'isOpen'}`}>
          ...
          <ul className="select-modal__list">
            {rubrics && rubrics.map((rubric) => (
              <li 
                ...
                value={rubric.CODE}
                key={rubric.ID}
                onClick={() => handleClickLi(rubric.ID)}>{rubric.NAME}</li>
            ))}
          </ul>
      </div>
      )}
